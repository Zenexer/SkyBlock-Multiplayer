...
public class SkyBlockMultiplayer extends JavaPlugin
{
	...
	public void loadConfig()
	{
		...
		else
		{
			...
			try
			{
				Data.ISLAND_DISTANCE = Integer.parseInt(this.getStringbyPath(this.configPlugin, this.fileConfig,
						"islandDistance", 50));
			...
			String[] dataItems = this.getStringbyPath(this.configPlugin, this.fileConfig,
					Config.OPTIONS_CHESTITEMS.path, items).split(";");
			...
			for (String s : dataItems)
			{
				if (s.trim() != "")
				{
					...
					{
						alitemsChest
								.add(new ItemStack(Integer.parseInt(dataValues[0]), Integer.parseInt(dataValues[1])));
					}
					catch (Exception ex)
					{
					}
			...
			Data.ITEMSCHEST = itemsChest;
			Data.SKYBLOCK_ONLINE = Boolean.parseBoolean(this.getStringbyPath(this.configPlugin, this.fileConfig,
					Config.OPTIONS_SKYBLOCKONLINE.path, true));
			Data.PVP = Boolean.parseBoolean(this.getStringbyPath(this.configPlugin, this.fileConfig,
					Config.OPTIONS_PVP.path, false));
			Data.LANGUAGE = this.getStringbyPath(this.configPlugin, this.fileConfig, Config.OPTIONS_LANGUAGE.path,
					"english");
		}
	}
	...
	public PlayerInfo getPlayer(Player p)
	{
		...
		pi.setHasIsland(Boolean.parseBoolean(this.getStringbyPath(this.configPlayer, this.filePlayer, path
				+ "hasIsland", false)));
		pi.setDead(Boolean.parseBoolean(this
				.getStringbyPath(this.configPlayer, this.filePlayer, path + "isDead", false)));
		pi.setIslandLocation(this.getLocationString(this.getStringbyPath(this.configPlayer, this.filePlayer, path
				+ "islandLocation", "")));
		pi.setOldPlayerLocation(this.getLocationString(this.getStringbyPath(this.configPlayer, this.filePlayer, path
				+ "oldLocation", this.getServer().getWorlds().get(0).getSpawnLocation())));
		pi.setIsOnIsland(Boolean.parseBoolean(this.getStringbyPath(this.configPlayer, this.filePlayer, path
				+ "isOnIsland", false)));
		return pi;
	}
	...
	public static World getSkyblockIslands()
	{
		...
		{
			skyblockIslands = WorldCreator.name(SkyBlockMultiplayer.WORLD_NAME).type(WorldType.NORMAL)
					.environment(Environment.NORMAL).generator(new SkyBlockChunkGenerator()).createWorld();
	...
	private boolean playerNewIsland(Player player, String s)
	{
		if (!Data.PVP)
		{
			...
			player.sendMessage(this.pNameChat + Language.MSGS_NEWISLANDPLAYER1.sentence + pi.getPlayerName()
					+ Language.MSGS_NEWISLANDPLAYER2.sentence);
			return true;
		}
		...
		Data.PLAYERS
				...
				.getPlayer()
				.sendMessage(
						this.pNameChat + Language.MSGS_GOTNEWISLAND1.sentence + player.getName()
								+ Language.MSGS_GOTNEWISLAND2.sentence);
		return true;
	}
			...
			if (!pi.getHasIsland())
			{
				...
				for (PlayerInfo p : Data.PLAYERS)
				{
					p.getPlayer().sendMessage(
							this.pNameChat + Language.MSGS_WELCOMEBROADCAST1.sentence + player.getName()
									+ Language.MSGS_WELCOMEBROADCAST2.sentence);
				}
	...
	private boolean playerJoin(Player player)
	{
		...
		player.teleport(SkyBlockMultiplayer.getSkyblockIslands().getSpawnLocation()); // Teleportiere
																						// Spieler
																						// zum
																						// Spawntower
																						// in
																						// der
																						// Mitte
		player.sendMessage(this.pNameChat + Language.MSGS_WELCOME1.sentence + islands + Language.MSGS_WELCOME2.sentence
				+ Data.PLAYERS_NUMBER + Language.MSGS_WELCOME3.sentence);
		return true;
	}
		...
		if (!Data.LANGUAGE.equalsIgnoreCase(s))
		{
			...
			if (f.exists())
			{
				...
				catch (Exception ex)
				{
					...
					sender.sendMessage(this.pNameChat + Language.MSGS_ERROROCCURED.sentence + ": "
