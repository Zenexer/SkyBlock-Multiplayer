...
public class CreateNewIsland
{
	...
	private void createIsland(Location l)
	{
		...
		for (int x = 2; x <= 4; x++)
		{
			for (int z = -1; z <= 1; z++)
			{
				SkyBlockMultiplayer.getSkyblockIslands().getBlockAt(x + l.getBlockX(), 62, z + l.getBlockZ())
		...
		// Erstelle Baum
		// SkyBlockMultiplayer.getSkyblockIslands().generateTree(new
		// Location(SkyBlockMultiplayer.getSkyblockIslands(), 5 + l.getBlockX(),
		// 64, l.getBlockZ()), TreeType.TREE);
		this.createTree(new Location(SkyBlockMultiplayer.getSkyblockIslands(), 5 + l.getBlockX(), 64, l.getBlockZ()));

		// Setze Bedrock
		SkyBlockMultiplayer.getSkyblockIslands().getBlockAt(l.getBlockX(), l.getBlockY() - 3, l.getBlockZ())
				...
				for (int x = -2; x <= 2; x++)
				{
					for (int z = -2; z <= 2; z++)
					{
						if (y != 4)
						{
							if (x != 0 || z != 0)
							{
								SkyBlockMultiplayer.getSkyblockIslands().getBlockAt(px + x, py + y, pz + z)
										.setType(itemLeaves.getType());
							}
						}
	...
	private boolean checkIfOccupied(Location l)
	{
		for (int x = -5; x <= 5; x++)
		{
			for (int y = -5; y <= 5; y++)
			{
				for (int z = -5; z <= 5; z++)
				{
					if (!SkyBlockMultiplayer.getSkyblockIslands()
							.getBlockAt(l.getBlockX() + x, l.getBlockY() + y, l.getBlockZ() + z).getType()
							.equals(Material.AIR))
					{
						return true;
					}
